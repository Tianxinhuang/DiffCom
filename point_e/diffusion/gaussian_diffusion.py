...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        #print(model_output.shape, model_output, self.sqrt_recipm1_alphas_cumprod)
        #assert False
        return {
            "mean": model_mean,
    ...
    def p_sample_loop_inter(
        ...
        if noise is not None:
            ...
            img = th.randn(*shape, device=device) * temp
        indices = list(range(self.num_timesteps))[::-1]
        indices = indices[self.num_timesteps-startidx:]
        #print(img.shape)
        #assert False
    ...
    def training_losses(
        ...
        if self.loss_type == "kl" or self.loss_type == "rescaled_kl":
            ...
            #terms["mse"] = mean_flat((target[:,:3] - model_output[:,:3]) ** 2)+0.1*mean_flat((target[:,3:] - model_output[:,3:]) ** 2)
        ...
        return terms
    def couple_losses(
        self, model, x_start, x_noise, t, model_kwargs=None, noise=None
    ) -> Dict[str, th.Tensor]:
        """
        Compute training losses for a single timestep.
        ...
        """
        x_start = self.scale_channels(x_start)
        x_noise = self.scale_channels(x_noise)
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = th.randn_like(x_start)
        x_t = self.q_sample(x_start, t, noise=noise)
        x_t_noise = self.q_sample(x_noise, t, noise=noise)
        #print(x_start, x_noise)
        #print(x_t, x_t_noise)
        #assert False
        ...
        if self.loss_type == "kl" or self.loss_type == "rescaled_kl":
            ...
            model_output = model(x_t, t, **model_kwargs)
            model_noutput = model(x_t_noise, t, **model_kwargs)

            if isinstance(model_output, tuple):
                model_output, extra = model_output
                model_noutput, _ = model_noutput
            else:
                extra = {}
            ...
            if self.model_var_type in [
                ...
                assert model_output.shape == (B, C * 2, *x_t.shape[2:])
                model_output, model_var_values = th.split(model_output, C, dim=1)
                model_noutput, _ = th.split(model_noutput, C, dim=1)
                # Learn the variance using the variational bound, but don't let
                # it affect our mean prediction.
                frozen_out = th.cat([model_output.detach(), model_var_values], dim=1)
                terms["vb"] = self._vb_terms_bpd(
