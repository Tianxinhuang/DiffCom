...
def add2dict(hdata,tid,cens,start=0):
    ...
    dismat=np.sqrt(np.sum(np.square(np.expand_dims(hdata[:,:3],axis=1)-np.expand_dims(cens,axis=0)),axis=-1))
...
def get_part_data(data,ptnum,split='train',centype='r',sparse=False,scale=0.5):
    ...
    if centype == 'r':
        ndata=data[np.random.choice(datanum,2048*4,replace=False), :3]
        cennum=int(cennum*scale)
    ...
    coors, length, center = get_normal(data[..., :3])
    data = np.concatenate([coors, data[...,3:]], axis=-1)
...
def get_normal(data,cir=True):
    if not cir:
        ...
        length=(dmax-dmin)/2
        center=(dmax+dmin)/2
        result=0.5*(result-center)/length
    else:
        #center=np.mean(data,axis=1,keepdims=True)
        dmax=np.max(data,axis=1,keepdims=True)
        dmin=np.min(data,axis=1,keepdims=True)
        ...
        length=np.expand_dims(r,axis=-1)
        #print(np.shape(para))
        result=0.5*(data-center)/length#+cen
    return result,length,center
